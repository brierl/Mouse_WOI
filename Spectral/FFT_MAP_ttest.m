function [tmap,p1,h1,h1_cc_rs]=FFT_MAP_ttest(fft_image_ms_group,group_index,isbrain2_aff,thresh,mice,oi)
% script to perform pixel-wise ttest on FFT maps, uses cluster extent
...
%       oi.nVx oi.nVy contrast
    ...
    for i=1:length(fft_image_ms_group)
        fft_image_ms_group{i}=fft_image_ms_group{i}.*isbrain2_aff;
    end
    % loop through pixels
    for f=1:size(fft_image_ms_group{1},1)
        for g=1:size(fft_image_ms_group{1},2)
                
            if length(group_index)==0 % if only 1 group, 1-sample t-test
                for w=1:length(oi.con_num) %loop through contrasts
                    if nansum(~isnan(fft_image_ms_group{1}(f,g,w,:)) & fft_image_ms_group{1}(f,g,w,:)~=0)<2
                        h1(f,g,w)=NaN;
                        p1(f,g,w)=NaN;
                        tmap(f,g,w)=NaN;
                    else
            ...
            elseif length(group_index)==1 %if 2 groups
                if strcmp(mice(1).test, '2-sample')
                    for w=1:length(oi.con_num) %loop through contrasts
                        if nansum(~isnan(fft_image_ms_group{1}(f,g,w,:)) & fft_image_ms_group{1}(f,g,w,:)~=0)<2 || nansum(~isnan(fft_image_ms_group{2}(f,g,w,:)) & fft_image_ms_group{2}(f,g,w,:)~=0)<2
                            h1(f,g,w)=NaN;
                            p1(f,g,w)=NaN;
                            tmap(f,g,w)=NaN;
                        else
    ...
    h1_cc=zeros(size(fft_image_ms_group{1},1)*size(fft_image_ms_group{1},2),length(oi.con_num));
